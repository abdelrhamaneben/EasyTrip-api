...
module.exports = {
  ...
  signup: function(req, res) {
    ...
    if (!req.param('name_first')) {
      console.log('param error: 1');
      return res.badRequest('');
    }

    if (!req.param('name_last')) {
      console.log('param error: 2');
      return res.badRequest('');
    }

    if (!req.param('email')) {
      console.log('param error: 3');
      return res.badRequest('');
    }

    if (!req.param('password')) {
      console.log('param error: 6');
      return res.badRequest('');
    }

    if (!req.param('role')) {
      console.log('param error: 5');
      return res.badRequest('');
    }

    if (req.param('role') != 'private') {
      if (!req.param('phone')) {
        console.log('param error: 4');
        return res.badRequest('');
      }

      if (!req.param('address_str_nbr')) {
        console.log('param error: 8');
        return res.badRequest('');
      }

      if (!req.param('address_str_name')) {
        console.log('param error: 9');
        return res.badRequest('');
      }

      if (!req.param('address_city')) {
        console.log('param error: 10');
        return res.badRequest('');
      }

      if (!req.param('address_code_zip')) {
        console.log('param error: 11');
        return res.badRequest('');
      }

      if (!req.param('address_country')) {
        console.log('param error: 12');
        return res.badRequest('');
      }
